...
const AudioCard = ({
  ...
  useEffect(() => {
    ...
    axios.post('https://kilo-app-backend.onrender.com/api/users/current-lesson', {
      lessonId: lesson._id,
    }, {
      headers: {
    ...
    const interval = setInterval(() => {
      ...
      if (audio && audio.duration) {
        ...
        if (currentProgress > 0) {
          ...
          axios.patch('https://kilo-app-backend.onrender.com/api/users/progress', {
            lessonId: lesson._id,
...
const togglePlay = () => {
  ...
  const onProgress = () => {
    ...
    if (percent >= 99 && lastToastLevel !== 'done') {
      ...
      axios.post('https://kilo-app-backend.onrender.com/api/user/mark-complete', {
        lessonId: lesson._id,
      }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  return (
    <div className={`card audio-card-container ${isCompleted ? 'lesson-completed' : 'pending'}`}>
      ...
      {lesson.thumbnail && (
        <img src={`https://kilo-app-backend.onrender.com${lesson.thumbnail}`} alt="Lesson" className="lesson-thumbnail" />
      )}
