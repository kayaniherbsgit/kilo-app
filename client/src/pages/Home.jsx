...
if (lastLessonId) {
  ...
  localStorage.removeItem('lastPlayedLessonId');
    ...
    if (user?.username) {
      axios.get(`http://${import.meta.env.VITE_API_URL}/api/users/state/${user.username}`).then(res => {
        setCompleted(res.data.completedLessons || []);
        setAudioProgress(res.data.audioProgress || {});
      });

      axios.get(`http://${import.meta.env.VITE_API_URL}/api/users/streak/${user.username}`).then(res => {
      ...
      axios.get(`http://${import.meta.env.VITE_API_URL}/api/notifications/unread/${user.username}`).then(res => {
        setUnreadCount(res.data.unreadCount || 0);
      });

      axios.get(`http://${import.meta.env.VITE_API_URL}/api/community/leaderboard`).then(res => {
...
const saveIndex = async (newIndex) => {
  ...
  if (lessonId) {
    ...
    await axios.post('http://${import.meta.env.VITE_API_URL}/api/users/current-lesson', {
      lessonId,
    }, {
