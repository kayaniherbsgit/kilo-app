...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    const res = await axios.get('https://kilo-app-backend.onrender.com/api/admin/stats', {
      headers: { Authorization: `Bearer ${token}` },
    });
    setStats(res.data);
  };
  ...
  const handleEditLesson = (lesson) => {
    ...
    setPreview(lesson.thumbnail ? `https://kilo-app-backend.onrender.com${lesson.thumbnail}` : null);
    setTab('upload');
    setModalOpen(true);
  };
  ...
  const handleUpdateLesson = async () => {
    ...
    await axios.put(`https://kilo-app-backend.onrender.com/api/lessons/${editLesson._id}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data', Authorization: `Bearer ${token}` },
    });
    ...
    if (window.confirm(`Delete ${selectedLessons.length} selected lessons?`)) {
      await Promise.all(
        selectedLessons.map(id => 
          axios.delete(`https://kilo-app-backend.onrender.com/api/lessons/${id}`, {
            headers: { Authorization: `Bearer ${token}` }
          })
        )
      );
        ...
        <div className="lessons-grid">
          ...
          <DragDropContext
            onDragEnd={(result) => {
              ...
              axios.post('https://kilo-app-backend.onrender.com/api/admin/reorder-lessons', {
